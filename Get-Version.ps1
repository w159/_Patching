function Get-Version {
    ...
    .PARAMETER MaximumRedirection
        Optional parameter to specify the maximum number of redirections to follow when using Invoke-WebRequest.
     ...
     process {
          if ($PsCmdlet.ParameterSetName -eq 'Uri') {
               try {
                    ...
                    if ($MaximumRedirection) {
                         $ParamHash.MaximumRedirection = $MaximumRedirection
                    }
          ...
          $String -split "`r`n" | ForEach-Object {
               if ($_ -match $Pattern) {
                    ...
                    Write-Warning "No version found within $_ using pattern $Pattern"
               }
          }
     }
