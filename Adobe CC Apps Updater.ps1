...
$AdobeRemoteUpdateManager_CHECK = Test-Path -Path 'C:\Windows\Utils\AdobeRUM\RemoteUpdateManager.exe'
...
if ($AdobeRemoteUpdateManager_CHECK -eq $false) {
    ...
    Invoke-WebRequest -UseBasicParsing 'https://onedrive.live.com/download?cid=AF928892EBCFB343&resid=AF928892EBCFB343%21198740&authkey=APGjFRM5LgIBCHg' `
        -OutFile 'C:\Windows\Utils\AdobeRemoteUpdateManager.zip' `
        -UserAgent $UserAgent

    New-Item C:\Utils -Name AdobeRUM -ItemType Directory -Force -ErrorAction SilentlyContinue
    Expand-Archive -Path 'C:\Windows\Utils\AdobeRemoteUpdateManager.zip' -DestinationPath C:\Windows\Utils\AdobeRUM
    Get-ChildItem -Path C:\Windows\Utils\AdobeRUM -Recurse | Unblock-File

    Start-Process -FilePath 'C:\Windows\Utils\AdobeRUM\RemoteUpdateManager.exe' -NoNewWindow -Wait
